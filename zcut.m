...
% % csv value rearrange! 
% ii = 11;
% % csv_phi = csvread('Vds_phi_0.0V_zcut.csv',1,0,[1 0 x_nod 1]);
% csv_phi = csvread('phi_zcut_Vgs_0.6V_Vds_0.05V.csv',1,0,[1 0 x_nod 1]);
% csv_nn  = csvread('nn_zcut_Vgs_0.6V_Vds_0.05V.csv',1,0,[1 0 x_nod 1]);
% csv_pp  = csvread('pp_zcut_Vgs_0.6V_Vds_0.05V.csv',1,0,[1 0 x_nod 1]);
% csv_mob = csvread('emob_zcut_Vgs_0.6V_Vds_0.05V.csv',1,0,[1 0 x_nod 1]);

%% phi plot
ii = 26;    % 14, 26, 38 (total dimension)
            % 9, 21, 33 (nn dimension)
...
%% nn plot
    ...
    figure('Color', [1,1,1]); % z-slice
    plot(x, n1(:,ii)*1e-6, 'b');
    xlabel('Position (nm)'); ylabel('Electron concentration'); 
    title('Electron concentration profile'); grid on;
    return;
        % notice: '??' should be changed! 
    figure('Color', [1,1,1]); % z-slice
    ...
    % notice: '??' should be changed! 
    figure('Color', [1,1,1]); % z-slice
    plot(x, nn1(:,ii)*1e-6, 'b');
    xlabel('Position (nm)'); ylabel('Electron concentration (cm^-3)'); 
    title('Electron concentration profile'); grid on;
    
        % notice: '??' should be changed! 
    figure('Color', [1,1,1]); % z-slice
    semilogy(x, n2(:,ii)*1e-6, 'b');
    xlabel('Position (nm)'); ylabel('Electron concentration (cm^-3)'); 
    title('Electron concentration profile'); grid on;

    % notice: '??' should be changed! 
    figure('Color', [1,1,1]); % z-slice
    semilogy(x, nn2(:,ii)*1e-6, 'b');
    xlabel('Position (nm)'); ylabel('Electron concentration (cm^-3)'); 
    title('Electron concentration profile'); grid on;
    
        % notice: '??' should be changed! 
    figure('Color', [1,1,1]); % z-slice
    semilogy(x, n3(:,ii)*1e-6, 'b');
    xlabel('Position (nm)'); ylabel('Electron concentration (cm^-3)'); 
    title('Electron concentration profile'); grid on;

    % notice: '??' should be changed! 
    figure('Color', [1,1,1]); % z-slice
    semilogy(x, nn3(:,ii)*1e-6, 'b');
    xlabel('Position (nm)'); ylabel('Electron concentration (cm^-3)'); 
    title('Electron concentration profile'); grid on;
...
% qphi = csv(:,2);
% % notice: '??' should be changed! 
% newphi = interp1(z,phi(ii,:)',qz);
% newphi(1) = phi(ii,1);
% newphi(end)= phi(ii,end);
% % notice: '??' should be changed! 
% figure('Color', [1,1,1]); % z-slice
% plot(qz, qphi, 'r', z, phi(ii,1:nz)', 'b');
