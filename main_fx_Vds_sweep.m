...
function [  ] = main_fx_Vds_sweep( Vg_bias, folder )
% clear;
...
for i = 1:51
    ...
    FF2_t = configue_Ffunction( 1, nodeNum, Vds(i), totalE);    
    FF1_l = configue_Ffunction( 2, nodeNum, 0, totalE);     % for #2 
    FF2_l = configue_Ffunction( 2, nodeNum, Vds(i), totalE);     % for #2 
...
%         return; 
        ...
        [ phi, nn ] = nLinPoisson2D( 100, jbase, phi, nn, Vgs);
...
%         disp(sprintf('Elapsed Time(sec): %03d \n', elapsedTime(i,j)));
    ...
    for k = 1:2
        ...
        % valley #2(t,l,t)        
        [E_v2, FF1_v2, FF2_v2] = find_Ffunction(Em_t, k, totalE, FF1_l, FF2_l);
        Ids(i) = Ids(i) + 2*negf_Current(2, Em_t, k, E_v2, FF1_v2, FF2_v2);
    end
    for k = 1:5
        % valley #3(t,t,l)
        [E_v3, FF1_v3, FF2_v3] = find_Ffunction(Em_l, k, totalE, FF1_t, FF2_t);
    ...
    if (Vds(i) - floor(Vds(i)*10)/10 < 1e-10) %% 0 V, 0.1 V, 0.2 V, 0.3 V, 0.4 V, 0.5 V
        ...
        save(sprintf('Drain\\%d\\Vgs_%.1f\\result_%03d.mat',folder, Vg_bias, i));
